...
if (isset($_POST['upload'])) {
    ...
    $content = trim($_POST['content_blog']);
    ...
    if (isset($_FILES['image_file']) && $_FILES['image_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($file_extension, $allowed_extensions)) {
            header('Location: blog_addform.php?status=error&message=' . urlencode('Hanya file JPG, JPEG, PNG, & WEBP yang diizinkan.'));
            exit;
        }
        ...
        if (move_uploaded_file($file_tmp_name, $destination_path)) {
            ...
            header('Location: blog_addform.php?status=error&message=' . urlencode('Gagal memindahkan file upload.'));
            exit;
        }
    }
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO blog (user_id, title_blog, content_blog, image_path, date_posted) VALUES (?, ?, ?, ?, NOW())");
        
        // Tipe bind_param: i = integer, sss = 3 string (title, content, image_path)
        $stmt->bind_param("isss", $_SESSION['user_id'], $title, $content, $image_path_to_db);
        ...
        if (!empty($keywords)) {
            $stmt_tag = $conn->prepare("INSERT IGNORE INTO tag (tag_name) VALUES (?)"); 
            $stmt_blog_tag = $conn->prepare("INSERT INTO blog_tag (blog_id, tag_id) VALUES (?, ?)");
            ...
            foreach ($keywords as $tag_name) {
                ...
                $tag_id_query = $conn->prepare("SELECT tag_id FROM tag WHERE tag_name = ?");
                ...
                if (!$stmt_blog_tag->execute()) {
                    if ($conn->errno !== 1062) { 
                        throw new Exception("Gagal insert blog_tag: " . $conn->error);
                    }
                }
            }
            $stmt_tag->close();
        ...
        $conn->commit();
        header('Location: blog_myblog.php?status=success');
        exit;
        ...
        header('Location: blog_addform.php?status=error&message=' . urlencode('Proses gagal: ' . $e->getMessage()));
        exit;
    }

} else {
    header('Location: blog_addform.php');
    exit;
}
?>